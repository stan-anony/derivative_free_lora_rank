...
from models.modeling_gpt_neo import GPTNeoForCausalLM
import deepspeed
...
with open("./ds_config.json", "rt") as f:
...
elif model_name in ['gpt2', 'gpt2-medium', 'gpt2-large', 'gpt2-xl','llama','gpt-neo']:
...
class LMForwardAPI:
    def __init__(self, model_name='roberta-large', task_name='sst2',
        ...
        if model_name in ['roberta-base', 'roberta-large']:
            ...
            self.model_name_or_path = "../llama_model/llama2/llama-2-7b"
            self.model = LlamaForCausalLM.from_pretrained(
                self.model_name_or_path,
                config=self.config,
            )
        elif model_name in ['gpt-neo']:
            self.config = GPTNeoConfig.from_pretrained("./gpt-neo")
            self.tokenizer = GPT2Tokenizer.from_pretrained("./gpt-neo")
            self.model_name_or_path = "./gpt-neo"
        ...
        if random_proj == 'normal':
            ...
            elif model_name in ['gpt2', 'gpt2-medium', 'gpt2-large', 'gpt2-xl','gpt-neo']:
    ...
    def calc_metric(self, logits, target):
        ...
        #interest_index = [x + 1 for x in interest_index]
            ...
            with torch.no_grad():
                ...
                if random_proj == 'normal' and len(self.intermediate_stats) == 1:
                    ...
                    for i, h in enumerate(hidden_states[1:-1]):
                        ...
                        hidden = hidden_states[i].clone().reshape(-1).detach().cpu().numpy()
