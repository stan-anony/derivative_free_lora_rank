...
class RobertaSelfAttention(nn.Module):
    ...
    def forward(
        ...
        lora = self.dropout(hidden_states) @ lora_A
        delta_lora_V = lora @ lora_B

        mixed_query_layer = mixed_query_layer + delta_lora_V
        lora_k = self.dropout(hidden_states) @ lora_KA
        delta_lora_K = lora_k @ lora_KB
        # If this is instantiated as a cross-attention module, the keys
        # and values come from an encoder; the attention mask needs to be
        # such that the encoder's padding tokens are not attended to.
        if encoder_hidden_states is not None:
            mixed_key_layer = self.key(encoder_hidden_states) + delta_lora_K
            mixed_value_layer = self.value(encoder_hidden_states)
            attention_mask = encoder_attention_mask
        else:
            mixed_key_layer = self.key(hidden_states) + delta_lora_K
...
class RobertaForMaskedLM(RobertaPreTrainedModel):
    ...
    def __init__(self, config, lora_r, inference_framework='pt', onnx_model_path=None):
        super().__init__(config)
